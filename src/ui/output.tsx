...
export const Output = ({ role, requirements, suggestedDepartments, isLoading }: OutputProps) => {
  ...
  return (
    <div className="flex min-h-[100%] flex-col gap-5 lg:max-w-[60%]">
      <div className="flex flex-col gap-2 rounded-xl bg-[#2F2F38] p-4">
        <>
          <span className="text-xl font-light uppercase text-[#B9B9CA]">Rola</span>
          <span>{role}</span>
        </>
        <span className="text-xl font-light uppercase text-[#B9B9CA]">Wymagania</span>
        <div className="flex flex-col">
          {requirements.map((requirement) => (
            <span key={requirement}>&#x2022; {requirement}</span>
          ))}
        </div>
        <span className="text-xl font-light uppercase text-[#B9B9CA]">Sugerowane działy</span>
        <div className="flex flex-col">
          {suggestedDepartments.map(({ departmentName, match }) => (
            <div className="flex gap-2" key={departmentName}>
              <span className={cn(getColorForMatch(match))}>{departmentName}</span>
              <span>-</span>
              <span>{match}%</span>
            </div>
          ))}
